...
(function(){
 ...
 if(!B)return;
 B.registerTile({
  id:1005,key:'BELT_LEFT',name:'⬅️ ベルト（左）',cat:'terrain',solid:true,
  sprite(ctx,x,y,size){
   ctx.fillStyle='#26364a';ctx.fillRect(x,y,size,size);
   ctx.fillStyle='#a78bfa';ctx.fillRect(x+1,y+size*.3,size-2,size*.4);
   ...
   ctx.strokeStyle='#8b5cf6';ctx.strokeRect(x+.5,y+.5,size-1,size-1);
  },
